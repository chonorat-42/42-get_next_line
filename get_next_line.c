...
/*   Updated: 2023/01/26 17:32:12 by chonorat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*prepare_for_next(char *stash)
{
	...
	prepared = ft_calloc((ft_strlen(&stash[index++])), sizeof(char));
	if (!prepared)
		return (free(stash), stash = NULL, NULL);
	index_j = 0;
	while (stash[index])
...
char	*stash_to_line(char *stash)
{
	...
	while (stash[index] != '\n' && stash[index])
		index++;
	stash_line = ft_calloc(index + 1 + ft_strchr(stash, '\n'), sizeof(char));
	if (!stash_line)
		return (NULL);
	index = 0;
	while (stash[index] && stash[index] != '\n')
...
char	*read_fd(int fd, char *stash, int read_switch)
{
	...
	while (read_switch > 0 && !ft_strchr(stash, '\n'))
	{
